...
def print_data(ground_points, webcam_points, translation_factors, w, h):
    print(ground_points[str(11)][0:2] * np.array([w, h]) - np.array(
        list(translation_factors)))
...
def compare_keypoints(ground_points, webcam_points, w, h, translation_factors):
    ...
    for i in range(len(ground_points)):
        ground_points_array.append(
            np.array(ground_points[str(i)])[0:2] * np.array([w, h]) -
            np.array(list(
                translation_factors)))  # only using x, y coordinates of keypoints

        webcam_points_array.append(
            np.array(webcam_points[i])[0:2] * np.array([w, h]))
...
def connect_points(points, translation_factors, image, image_shape, scale):
    ...
    for p in points_connect_dict:
        ...
        for endpoint in points_connect_dict[p]:
            ...
            cv2.line(image, (
            round(curr_point[0] * scale), round(curr_point[1] * scale)),
...
def put_text(image, text, h, w):
    image = cv2.putText(img=image, org=(w - 700, 50),
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2,
                        color=(0, 0, 0), text=text,
                        thickness=3)
    return image


def gen_frames():
    with open('metadata/' + "test1" + '.json') as f:
        data = json.load(f)
    ...
    while True and counter_start <= (max_count - 1):
        ...
        if not success:
            ...
            image = cv2.putText(img=image, org=(w // 2, h // 2),
                                fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=2,
                                color=(0, 0, 0),
                                text=str((max_count - counter_start) // 40),
                                thickness=2)
            ...
            if not success:
                ...
                if pose_landmarks is not None:
                    ...
                    pose_landmarks_str_keys = {str(i): [lmk.x, lmk.y, lmk.z]
                                               for i, lmk in enumerate(
                            pose_landmarks.landmark)}
                    pose_landmarks = {i: [lmk.x, lmk.y, lmk.z]
                                      for i, lmk in
                                      enumerate(pose_landmarks.landmark)}

                    data[counter] = {
                        i: [scale_t * coord for coord in data[counter][i]] for i
                        in data[counter]}
                    ...
                    thingy = connect_points(pose_landmarks_str_keys, (0, 0),
                                            thingy, (h, w), scale=0.1)

                    points = compare_keypoints(data[counter], pose_landmarks, w,
                                               h, (x_t, y_t))
                    ...
                    thingy = put_text(thingy, "Score :" + str(
                        round(sum(avg) / len(avg), 2)), h, w)
...
def train():
    ...
    return render_template('dancepage.html', numDir=numDir, numFiles=numFiles,
                           dances=dances)
